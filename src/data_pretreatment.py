...
if __name__ == '__main__':
    ...
    """distance = cal_euclidean_matrix(features)
    dfs = pd.read_excel('./data/assembled_molecules.xlsx')
    cal_fingerprints(dfs)
    matrix = cal_fps_similarity_matrix(dfs)"""
    with open('./data/similarity_matrix.pickle', 'rb') as file:
        dis_matrix = pickle.load(file)
    selected_examples = ks_selection(dis_matrix, n_examples=20)
    print('Finished')
